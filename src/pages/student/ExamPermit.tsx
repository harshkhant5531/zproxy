...
import { Ticket, Download, Lock, CheckCircle2, XCircle, Loader2, ShieldCheck, Fingerprint } from "lucide-react";
  ...
  return (
    <div className="space-y-8 max-w-2xl mx-auto py-8">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-black tracking-tighter text-white uppercase italic">Nexus Permit</h1>
        <p className="text-[10px] text-slate-500 font-mono tracking-[0.3em] uppercase">Examination Access Authorization // HID-V4</p>
      </div>
      ...
      <div className="relative group">
        ...
        <Card className={`relative bg-slate-950/80 border-2 backdrop-blur-xl shadow-2xl overflow-hidden ${overallEligible ? "border-primary/20" : "border-rose-500/20"}`}>
          ...
          <CardContent className="p-8 space-y-8">
            <div className="flex justify-between items-start border-b border-slate-800 pb-6">
              <div className="space-y-1">
                <p className="text-[9px] font-black text-primary uppercase tracking-[0.4em]">AURA INTEGRITY REPOSITORY</p>
                <h2 className="text-xl font-black text-white italic uppercase tracking-tight">End Term Protocol — 2026</h2>
                <div className="flex items-center gap-2 mt-2">
                  <div className={`h-1.5 w-10 rounded-full ${overallEligible ? "bg-primary shadow-[0_0_8px_rgba(34,211,238,0.5)]" : "bg-rose-500"}`} />
                  <span className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">{overallEligible ? "Status: Authorized" : "Status: Inhibited"}</span>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-12">
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Subject Identity</label>
                <p className="text-sm font-bold text-slate-200 uppercase">{user?.profile?.fullName || user?.username}</p>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Serial Index</label>
                <p className="text-sm font-mono text-slate-300"># {user?.profile?.studentId || "STU-000"}</p>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Assigned Sector</label>
                <p className="text-sm font-bold text-slate-200 uppercase">{user?.profile?.department || "General Computation"}</p>
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">Temporal Cycle</label>
                <p className="text-sm font-bold text-slate-200">Segment 0{user?.profile?.semester || 5}</p>
              </div>
            </div>

            <div className="space-y-4 pt-4 border-t border-slate-900">
              <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em]">Course Integrity Metrics</p>
              <div className="grid gap-2">
                {courseEligibility.map(course => (
                  <div key={course.id} className="flex items-center justify-between rounded-lg bg-slate-900/60 border border-slate-800 p-3 hover:border-slate-700 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className={`h-1.5 w-1.5 rounded-full ${course.eligible ? "bg-primary animate-pulse" : "bg-rose-500"}`} />
                      <div>
                        <p className="text-xs font-black text-slate-200 uppercase tracking-tight">{course.id} // {course.name}</p>
                        <p className="text-[10px] text-slate-500 font-mono">INTEGRITY: {course.attendance}%</p>
                      </div>
                    </div>
                    {course.eligible ? (
                ...
                {courseEligibility.length === 0 && <p className="text-center py-4 text-xs text-slate-600 italic">No course data indexed</p>}
              </div>
            </div>
